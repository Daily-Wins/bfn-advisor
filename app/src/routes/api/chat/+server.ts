...
import { routeQuestionSemantic, resolveScope } from '$lib/server/semantic-router';
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	for (const m of history) {
		if (!m || typeof m !== 'object')
			return json({ error: 'invalid_history_entry' }, { status: 400 });
	...
	const matches = await routeQuestionSemantic(routingQuery, scope);
