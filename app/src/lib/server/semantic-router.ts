...
function detectRegulationBoost(question: string): string | null {
	...
	if (/\bbokföring(slagen|snämnden)?\b/.test(q) && !/\b(års)?redovisning/.test(q))
		return 'Bokföring';
	return null;
}
...
function scopeRegulations(scope?: RegulationScope): Set<string> | null {
	if (!scope || scope.type === 'auto') return null;
	if (scope.type === 'single') return new Set([scope.regulation]);
	if (scope.type === 'comparison') return new Set(scope.regulations);
	return null;
}
...
async function getSemanticResults(
	...
	const [queryEmbedding, allSections] = await Promise.all([
		getQueryEmbedding(question),
		getEmbeddings(),
	]);

	const allowed = scopeRegulations(scope);
	const sections = allowed ? allSections.filter((s) => allowed.has(s.regulation)) : allSections;
 ...
 */
function reciprocalRankFusion(lists: ChapterMatch[][], k = 60): ChapterMatch[] {
...
export function filterMatchesByScope(
	...
	if (scope.type === 'single') {
		return matches.filter((m) => m.regulation === scope.regulation);
	}
	if (scope.type === 'comparison') {
		return matches.filter((m) => scope.regulations.includes(m.regulation));
	}
	return matches;
}

export async function routeQuestionSemantic(
	question: string,
	scope?: RegulationScope,
): Promise<ChapterMatch[]> {
	...
	try {
		const [semanticResults, keywordResults] = await Promise.all([
			getSemanticResults(question, scope),
			Promise.resolve(routeQuestion(question)),
		]);

		const filteredKeyword = scope ? filterMatchesByScope(keywordResults, scope) : keywordResults;

		const fused = reciprocalRankFusion([semanticResults, filteredKeyword]);
		...
		// Embedding API failed — fall back to keyword router only (scope-filtered)
		const keywordResults = routeQuestion(question);
		return scope ? filterMatchesByScope(keywordResults, scope) : keywordResults;
	}
