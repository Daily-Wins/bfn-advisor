...
describe('filterMatchesByScope', () => {
  ...
  it('filters to comparison regulations', () => {
    const result = filterMatchesByScope(mixedMatches, { type: 'comparison', regulations: ['K2', 'K3'] });
    expect(result).toHaveLength(3);
    expect(result.every(m => m.regulation === 'K2' || m.regulation === 'K3')).toBe(true);
  });

  it('falls back to all matches when no matches for single regulation', () => {
    const result = filterMatchesByScope(mixedMatches, { type: 'single', regulation: 'Fusioner' });
    expect(result).toEqual(mixedMatches);
  });

  it('falls back to all matches when no matches for comparison regulations', () => {
    const result = filterMatchesByScope(mixedMatches, { type: 'comparison', regulations: ['Fusioner', 'BRF'] });
    expect(result).toEqual(mixedMatches);
  });
